...
import { Plus, Pin, Pencil, Trash2, X, Megaphone, FileText, Download, FolderOpen } from 'lucide-react'
...
export default function AvisosYDocumentosPage() {
  ...
  function openNewAviso() {
    setAvisoTitle(''); setAvisoContent(''); setAvisoPinned(false); setEditAvisoId(null); setShowAvisoForm(true)
  }
  function openEditAviso(a: Announcement) {
    setAvisoTitle(a.title); setAvisoContent(a.content); setAvisoPinned(a.is_pinned); setEditAvisoId(a.id); setShowAvisoForm(true)
  }
  async function saveAviso() {
    if (!avisoTitle.trim() || !avisoContent.trim()) return
    setSavingAviso(true)
    const payload = { title: avisoTitle.trim(), content: avisoContent.trim(), is_pinned: avisoPinned, published: true }
        ...
        <div className="rounded-xl border-2 bg-white p-6 mb-6" style={{ borderColor: 'var(--blue-action)' }}>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={saveAviso} disabled={savingAviso || !avisoTitle.trim() || !avisoContent.trim()}
                className="flex-1 py-2.5 rounded-lg text-sm font-semibold text-white disabled:opacity-50"
                style={{ backgroundColor: 'var(--blue-action)' }}>{savingAviso ? 'Guardando...' : editAvisoId ? 'Guardar' : 'Publicar'}</button>
            </div>
          </div>
        </div>
      )}
